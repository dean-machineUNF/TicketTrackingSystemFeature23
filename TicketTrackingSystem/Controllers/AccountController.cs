...
using static Microsoft.Owin.IOwinContext;
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(string contractToken)
        {
            using (var db = new TicketTrackingSystemContext())
            {
                ...
                if (guidToCheck == new Guid(ConfigurationManager.AppSettings["CreateAdminToken"]))
                {
                    return RedirectToAction("RegisterAdmin", new { contractToken });
                }
        ...
        */
        [AllowAnonymous]
        public ActionResult RegisterAdmin(string adminAuth)
        {
            if (adminAuth == "Jags")
            {
                return View();
            ...
            {
                return RedirectToAction("UnAuthorizedRegister");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    using (var db = new TicketTrackingSystemContext())
                    {
                        ...
                        //create their companyUser relation
                        /*
                        var newCompanyUserRecord = new CompanyUser()
                        {
                            UserId = user.Id,
                            CompanyId = model.CompanyId
                        };

                        db.CompanyUsers.Add(newCompanyUserRecord);
                        */
                        db.SaveChanges();
                    }
